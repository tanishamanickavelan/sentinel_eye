...
VIDEO_CLIP_FOLDER = os.path.join(BASE_DIR, 'alerts', 'clips')
IMAGE_FOLDER = os.path.join(BASE_DIR, 'alerts', 'screenshots')
...
def save_clip(video_frames, confidence_score, screenshot_path):
    if confidence_score < ALERT_CONFIDENCE_THRESHOLD:
        print(f"[INFO] Skipping save: Final prediction {confidence_score:.4f} below threshold {ALERT_CONFIDENCE_THRESHOLD}.")
        return
    if not video_frames:
    ...
    while cap.isOpened():
        ...
        if RECORDING_ACTIVE:
            ...
            if elapsed_recording_time >= CLIP_DURATION_SECONDS or len(RECORDING_FRAMES) >= CLIP_DURATION_SECONDS * 20:
                save_clip(RECORDING_FRAMES, LAST_PREDICTION, SAVED_SCREENSHOT_PATH)
                RECORDING_ACTIVE = False
                RECORDING_FRAMES = []
                ALERT_START_TIME = None
                SCREENSHOT_TAKEN = False
